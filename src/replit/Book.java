...
public class Book {
    ...
    public static String capitalize(String word) {
...

        {
            ...
            word = wordNew.toUpperCase().substring(0, 1) + wordNew.toLowerCase().substring(1);
        }
            ...
            for (int i = 0; i<title.length ; i++){
                if (!isTitleWord(title[i]) && i!=0)
                    title[i] = title[i].toLowerCase();
                else
                    title[i] = title[i].toUpperCase().substring(0, 1) + title[i].toLowerCase().substring(1);
    ...
    public static boolean isTitleWord(String word) {
        String [] articles =  { "a" , "an", "the","and", "but", "for", "nor", "or", "so", "yet","at",
        ...
        for (int i = 0; i<articles.length ; i++) {
            if (word.equals(articles[ i ].trim()))
                check = true;
        }

        return (oneWord && !check);
